...
const PLANS = [
  { id:'starter', name:'Starter', mo:29, yr:23, desc:'Perfect for solo founders', href:'/signup', cta:'Get started', features:['1 website scan/month','Compliance risk report','Privacy policy generation','Cookie policy generation','Email support'] },
  { id:'growth',  name:'Growth',  mo:79, yr:63, desc:'For actively growing businesses', href:'/signup', cta:'Start free trial', popular:true, features:['5 scans/month','Full document suite','Compliance monitoring','Regulatory change alerts','AI compliance assistant','Priority support'] },
  { id:'agency',  name:'Agency',  mo:199,yr:159,desc:'For agencies with multiple clients', href:'/signup', cta:'Get started', features:['Unlimited scans','Multi-client dashboard','White-label reports','API access','DPA templates','Dedicated support'] },
  { id:'enterprise',name:'Enterprise',mo:500,yr:400,desc:'Custom compliance infrastructure', href:'/contact', cta:'Contact us', features:['Everything in Agency','Custom integrations','99.9% SLA','Legal review add-on','SSO authentication','Account manager'] },
]
...
const FAQS = [
  ...
  {q:'How accurate is the compliance scanner?',a:'Our scanner checks for the most common GDPR issues including cookies, consent mechanisms, privacy policy presence, and data forms. It catches the vast majority of issues SMEs face.'},
  ...
  {q:'What regulations do you cover?',a:'GDPR (EU), UK Data Protection Act 2018, ePrivacy Regulations (Cookie Law), and ICO guidance. US CCPA coming soon.'},
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:14}}>
            {PLANS.map(p=>(
              <div key={p.id} style={{background:p.popular?'var(--green)':'var(--white)',border:p.popular?'none':'1px solid var(--border)',borderRadius:18,padding:24,position:'relative',display:'flex',flexDirection:'column'}}>
                ...
                <a href={p.href} style={{display:'block',textAlign:'center',padding:'12px',borderRadius:9,fontSize:13,fontWeight:500,textDecoration:'none',background:p.popular?'var(--lime)':'var(--green-p)',color:p.popular?'var(--ink)':'var(--green)',border:p.popular?'none':'1px solid var(--green-m)'}}>{p.cta}</a>
              </div>
            ))}
          </div>
        </div>
...
function FaqItem({q,a}) {
  ...
  return (
    <div style={{border:'1px solid var(--border)',borderRadius:11,overflow:'hidden',marginBottom:2}}>
      <button onClick={()=>setOpen(o=>!o)} style={{width:'100%',display:'flex',justifyContent:'space-between',alignItems:'center',padding:'16px 20px',background:open?'var(--green-p)':'var(--white)',border:'none',cursor:'pointer',textAlign:'left',fontSize:14,fontWeight:500,color:'var(--ink)',transition:'background .2s',gap:12}}>
