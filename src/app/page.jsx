...
function EarlyAccess() {
  ...
  return (
    <section style={{padding:'88px 0',background:'linear-gradient(135deg, rgba(0,212,170,0.12) 0%, rgba(124,58,237,0.1) 100%)',position:'relative',overflow:'hidden'}}>
      <div className="orb orb-teal" style={{width:500,height:500,top:'-20%',left:'60%',opacity:0.5}}/>
      <ParticleCanvas count={40} speed={0.2} connectDist={100}/>
      <div className="wrap" style={{position:'relative',zIndex:1}}>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:64,alignItems:'center'}}>
          <AnimateOnScroll direction="left">
            <span style={{fontSize:11,fontWeight:600,letterSpacing:'.12em',textTransform:'uppercase',color:'var(--accent)',marginBottom:12,display:'block'}}>Early access</span>
            <h2 style={{fontFamily:'Syne,sans-serif',fontSize:'clamp(24px,3vw,38px)',fontWeight:700,color:'var(--ink)',marginBottom:16,lineHeight:1.15}}>AlgoGrass is live — start for free today</h2>
            <p style={{fontSize:15,color:'var(--ink2)',lineHeight:1.75,fontWeight:300,marginBottom:24}}>Our full compliance platform is available now. Scan your website, generate documents, and track your compliance score — all in one place.</p>
            {['✓  Free compliance scanner — available right now','✓  AI document generation — live now','✓  Compliance dashboard — live now','✓  Early access members get 3 months free on annual plan'].map(t=>(
              <div key={t} style={{fontSize:14,color:'var(--ink2)',marginBottom:8}}>{t}</div>
            ))}
          </AnimateOnScroll>
          <AnimateOnScroll direction="right">
            <TiltCard intensity={6}>
              <div style={{background:'rgba(13,21,37,0.85)',border:'1px solid rgba(0,212,170,0.2)',borderRadius:18,padding:'36px 32px',backdropFilter:'blur(16px)'}}>
                {done ? (
                  <div style={{textAlign:'center',padding:'24px 0'}}>
                    <div style={{fontSize:48,marginBottom:16}} className="animate-float">🎉</div>
                    <h3 style={{fontFamily:'Syne,sans-serif',fontSize:20,fontWeight:700,color:'var(--ink)',marginBottom:10}}>You are on the list!</h3>
                    <p style={{fontSize:14,color:'var(--ink2)',marginBottom:20}}>We have saved your spot. Try the free scanner now.</p>
                    <a href="/scan" className="btn btn-primary">Try the free scanner →</a>
                  </div>
                ) : (
                  <>
                    <h3 style={{fontFamily:'Syne,sans-serif',fontSize:20,fontWeight:700,color:'var(--ink)',marginBottom:8}}>Join the early access list</h3>
                    <p style={{fontSize:13,color:'var(--ink2)',marginBottom:22,lineHeight:1.6}}>Get 3 months free on any annual plan. No spam. Unsubscribe any time.</p>
                    <input type="email" placeholder="your@email.com" value={email} onChange={e=>setEmail(e.target.value)} onKeyDown={e=>e.key==='Enter'&&join()}
                      style={{width:'100%',border:'1.5px solid rgba(0,212,170,0.25)',borderRadius:9,padding:'12px 14px',fontSize:14,background:'rgba(255,255,255,0.05)',color:'var(--ink)',outline:'none',marginBottom:12,boxSizing:'border-box',transition:'border-color 0.3s'}}
                      onFocus={e=>e.currentTarget.style.borderColor='rgba(0,212,170,0.6)'}
                      onBlur={e=>e.currentTarget.style.borderColor='rgba(0,212,170,0.25)'}/>
                    {error&&<p style={{fontSize:12,color:'#f87171',marginBottom:8}}>{error}</p>}
                    <button onClick={join} disabled={loading} className="btn btn-primary btn-full">{loading?'Saving...':'Join early access →'}</button>
                    <p style={{fontSize:11,color:'var(--ink2)',marginTop:10,textAlign:'center',opacity:0.6}}>Your email will only be used for AlgoGrass updates.</p>
                  </>
                )}
              </div>
            </TiltCard>
...
function CtaBanner() {
  return (
    <section style={{padding:'88px 0',position:'relative',overflow:'hidden'}}>
      <div className="orb orb-purple" style={{width:600,height:600,top:'-30%',left:'50%',transform:'translateX(-50%)',opacity:0.5}}/>
      <div className="orb orb-teal" style={{width:300,height:300,bottom:'-10%',right:'10%',opacity:0.4}}/>
      <ParticleCanvas count={30} speed={0.15} connectDist={90}/>
      <div className="wrap" style={{maxWidth:640,position:'relative',zIndex:1}}>
        <AnimateOnScroll direction="scale">
          <div style={{textAlign:'center',padding:'56px 48px',background:'rgba(13,21,37,0.7)',border:'1px solid rgba(0,212,170,0.15)',borderRadius:24,backdropFilter:'blur(20px)'}} className="glow-border">
            ...
            <p style={{fontSize:11,color:'var(--ink2)',marginTop:16,opacity:0.6}}>AlgoGrass — The GDPR compliance platform trusted by UK & EU businesses.</p>
          </div>
        </AnimateOnScroll>
      </div>
    </section>
