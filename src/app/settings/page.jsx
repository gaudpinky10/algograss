...
const S = {
  ...
  row: { marginBottom: 18 },
  btn: { background: 'linear-gradient(135deg,#0EA5E9,#0284C7)', color: '#FFFFFF', fontWeight: 700, fontSize: 14, padding: '11px 26px', borderRadius: 10, border: 'none', cursor: 'pointer' },
...
export default function SettingsPage() {
  ...
  const [form, setForm] = useState({ name: '', website: '', brandName: '', brandLogo: '', brandColor: '#0EA5E9', removeBranding: false })
  ...
  useEffect(() => {
    ...
    fetch('/api/user/settings').then(r => r.json()).then(d => {
      if (d.settings) {
        const s = d.settings
        setSettings(s)
        setForm(f => ({ ...f, name: s.name || f.name, website: s.website || f.website, brandName: s.brandName || '', brandLogo: s.brandLogo || '', brandColor: s.brandColor || '#0EA5E9', removeBranding: s.removeBranding || false }))
  ...
  return (
    <div style={S.page}>
      <div style={S.wrap}>
        ...
        {/* Tabs */}
        <div style={{ display: 'flex', gap: 4, marginBottom: 24, background: '#F8FAFC', border: '1px solid rgba(15,23,42,0.1)', borderRadius: 12, padding: 4 }}>
          {TABS.map(t => (
            <button key={t} onClick={() => { setTab(t); setMsg(''); setErr('') }} style={{ flex: 1, padding: '9px 12px', borderRadius: 9, fontSize: 13, fontWeight: 600, border: 'none', cursor: 'pointer', background: tab === t ? 'rgba(14,165,233,0.12)' : 'transparent', color: tab === t ? '#0EA5E9' : '#94A3B8' }}>{t}</button>
          ))}
        </div>

        {msg && <div style={{ background: 'rgba(14,165,233,0.08)', border: '1px solid rgba(14,165,233,0.25)', borderRadius: 10, padding: '12px 16px', marginBottom: 16, color: '#0EA5E9', fontSize: 14 }}>✅ {msg}</div>}
          ...
          <div style={S.card}>
            ...
            <div style={S.row}><label style={S.label}>Plan</label>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <span style={{ background: isPro ? 'rgba(14,165,233,0.12)' : 'rgba(124,158,255,0.12)', color: isPro ? '#0EA5E9' : '#7C9EFF', fontWeight: 700, fontSize: 13, padding: '4px 14px', borderRadius: 20 }}>{(user?.plan || 'free').toUpperCase()}</span>
                {!isPro && <a href="/pricing" style={{ color: '#0EA5E9', fontSize: 13, fontWeight: 600 }}>Upgrade to Pro →</a>}
              </div>
            </div>
          ...
          <div style={S.card}>
            {!isPro && (
              <div style={{ background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.25)', borderRadius: 10, padding: '14px 16px', marginBottom: 20 }}>
                ...
                <a href="/pricing" style={{ color: '#0EA5E9', fontSize: 13, fontWeight: 600 }}>Upgrade to Pro →</a>
              </div>
            )}
            ...
            {form.brandLogo && <div style={{ ...S.row, background: '#FFFFFF', border: '1px solid rgba(15,23,42,0.1)', borderRadius: 10, padding: 16, marginBottom: 18 }}><img src={form.brandLogo} alt="Logo preview" style={{ maxHeight: 60, maxWidth: 200 }} onError={e => e.target.style.display='none'} /></div>}
            <div style={S.row}><label style={S.label}>Brand colour</label>
              <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                <input type="color" value={form.brandColor} onChange={e => setForm(f => ({ ...f, brandColor: e.target.value }))} style={{ width: 44, height: 44, border: '1px solid rgba(15,23,42,0.1)', borderRadius: 8, cursor: 'pointer', background: 'none', padding: 2 }} disabled={!isPro} />
          ...
          <div style={S.card}>
            ...
            {[
              ...
              <div key={key} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 0', borderBottom: '1px solid rgba(30,45,69,0.5)' }}>
                ...
                <div onClick={() => setNotif(n => ({ ...n, [key]: !n[key] }))} style={{ width: 44, height: 24, borderRadius: 12, background: notif[key] ? '#0EA5E9' : 'rgba(15,23,42,0.1)', cursor: 'pointer', position: 'relative', transition: 'background 0.2s', flexShrink: 0 }}>
