...
const S = {
  ...
  h1: { fontSize: 30, fontWeight: 800, color: '#E8F0FE', margin: '0 0 6px', letterSpacing: -0.5 },
  ...
  row: { marginBottom: 18 },
  btn: { background: 'linear-gradient(135deg,#00D4AA,#00A882)', color: '#06111E', fontWeight: 700, fontSize: 14, padding: '11px 26px', borderRadius: 10, border: 'none', cursor: 'pointer' },
...
export default function SettingsPage() {
  ...
  const [form, setForm] = useState({ name: '', website: '', brandName: '', brandLogo: '', brandColor: '#00D4AA', removeBranding: false })
  ...
  useEffect(() => {
    ...
    fetch('/api/user/settings').then(r => r.json()).then(d => {
      if (d.settings) {
        const s = d.settings
        setSettings(s)
        setForm(f => ({ ...f, name: s.name || f.name, website: s.website || f.website, brandName: s.brandName || '', brandLogo: s.brandLogo || '', brandColor: s.brandColor || '#00D4AA', removeBranding: s.removeBranding || false }))
  ...
  return (
    <div style={S.page}>
      <div style={S.wrap}>
        ...
        {/* Tabs */}
        <div style={{ display: 'flex', gap: 4, marginBottom: 24, background: '#0D1525', border: '1px solid #1e2d45', borderRadius: 12, padding: 4 }}>
          {TABS.map(t => (
            <button key={t} onClick={() => { setTab(t); setMsg(''); setErr('') }} style={{ flex: 1, padding: '9px 12px', borderRadius: 9, fontSize: 13, fontWeight: 600, border: 'none', cursor: 'pointer', background: tab === t ? 'rgba(0,212,170,0.12)' : 'transparent', color: tab === t ? '#00D4AA' : '#94A3B8' }}>{t}</button>
          ))}
        </div>

        {msg && <div style={{ background: 'rgba(0,212,170,0.08)', border: '1px solid rgba(0,212,170,0.25)', borderRadius: 10, padding: '12px 16px', marginBottom: 16, color: '#00D4AA', fontSize: 14 }}>✅ {msg}</div>}
        ...
        {tab === 'Profile' && (
          <div style={S.card}>
            <h3 style={{ color: '#E8F0FE', fontWeight: 700, fontSize: 16, margin: '0 0 20px' }}>Profile Details</h3>
            ...
            <div style={S.row}><label style={S.label}>Plan</label>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <span style={{ background: isPro ? 'rgba(0,212,170,0.12)' : 'rgba(124,158,255,0.12)', color: isPro ? '#00D4AA' : '#7C9EFF', fontWeight: 700, fontSize: 13, padding: '4px 14px', borderRadius: 20 }}>{(user?.plan || 'free').toUpperCase()}</span>
                {!isPro && <a href="/pricing" style={{ color: '#00D4AA', fontSize: 13, fontWeight: 600 }}>Upgrade to Pro →</a>}
              </div>
            </div>
          ...
          <div style={S.card}>
            {!isPro && (
              <div style={{ background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.25)', borderRadius: 10, padding: '14px 16px', marginBottom: 20 }}>
                ...
                <a href="/pricing" style={{ color: '#00D4AA', fontSize: 13, fontWeight: 600 }}>Upgrade to Pro →</a>
              </div>
            )}
            <h3 style={{ color: '#E8F0FE', fontWeight: 700, fontSize: 16, margin: '0 0 20px' }}>White Label Branding</h3>
            ...
            {form.brandLogo && <div style={{ ...S.row, background: '#060B14', border: '1px solid #1e2d45', borderRadius: 10, padding: 16, marginBottom: 18 }}><img src={form.brandLogo} alt="Logo preview" style={{ maxHeight: 60, maxWidth: 200 }} onError={e => e.target.style.display='none'} /></div>}
            <div style={S.row}><label style={S.label}>Brand colour</label>
              <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                <input type="color" value={form.brandColor} onChange={e => setForm(f => ({ ...f, brandColor: e.target.value }))} style={{ width: 44, height: 44, border: '1px solid #1e2d45', borderRadius: 8, cursor: 'pointer', background: 'none', padding: 2 }} disabled={!isPro} />
        ...
        {tab === 'Notifications' && (
          <div style={S.card}>
            <h3 style={{ color: '#E8F0FE', fontWeight: 700, fontSize: 16, margin: '0 0 20px' }}>Email Notifications</h3>
            {[
              ...
              <div key={key} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 0', borderBottom: '1px solid rgba(30,45,69,0.5)' }}>
                <div>
                  <div style={{ fontSize: 14, fontWeight: 600, color: '#E8F0FE', marginBottom: 2 }}>{label}</div>
                  <div style={{ fontSize: 12, color: '#94A3B8' }}>{desc}</div>
                </div>
                <div onClick={() => setNotif(n => ({ ...n, [key]: !n[key] }))} style={{ width: 44, height: 24, borderRadius: 12, background: notif[key] ? '#00D4AA' : '#1e2d45', cursor: 'pointer', position: 'relative', transition: 'background 0.2s', flexShrink: 0 }}>
        ...
        {tab === 'Security' && (
          <>
            <div style={S.card}>
              <h3 style={{ color: '#E8F0FE', fontWeight: 700, fontSize: 16, margin: '0 0 20px' }}>Change Password</h3>
