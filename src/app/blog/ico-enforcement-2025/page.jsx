...
const StatGrid=({stats})=><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))',gap:16,marginBottom:32,marginTop:8}}>{stats.map(s=><div key={s.label} style={{background:'rgba(139,92,246,0.06)',border:'1px solid rgba(139,92,246,0.18)',borderRadius:12,padding:'18px 16px',textAlign:'center'}}><div style={{fontFamily:'var(--f-head)',fontSize:26,fontWeight:800,color:'var(--accent)',letterSpacing:'-0.02em'}}>{s.val}</div><div style={{fontSize:12,color:'var(--ink2)',marginTop:4,lineHeight:1.4}}>{s.label}</div></div>)}</div>
