...
const S = {
  ...
  textarea: { width: '100%', background: '#06060F', border: '1.5px solid rgba(255,255,255,0.09)', borderRadius: 10, padding: '10px 14px', color: '#0F172A', fontSize: 14, outline: 'none', resize: 'vertical', minHeight: 90, boxSizing: 'border-box', fontFamily: 'inherit' },
  row: { marginBottom: 18 },
  btn: { background: 'linear-gradient(135deg,#9B7BFA,#7C3AED)', color: '#06060F', fontWeight: 700, fontSize: 14, padding: '12px 28px', borderRadius: 10, border: 'none', cursor: 'pointer', width: '100%' },
  btnOutline: { background: 'transparent', color: '#9B7BFA', fontWeight: 600, fontSize: 13, padding: '10px 20px', borderRadius: 10, border: '1px solid #9B7BFA', cursor: 'pointer' },
  ...
  letter: { background: '#06060F', border: '1px solid rgba(255,255,255,0.09)', borderRadius: 12, padding: 24, color: '#CBD5E1', fontSize: 13, lineHeight: 1.8, whiteSpace: 'pre-wrap', fontFamily: 'Georgia, serif' },
}
...
function Countdown({ breachDateTime }) {
  ...
  const col = hrs > 24 ? '#9B7BFA' : hrs > 6 ? '#F59E0B' : '#EF4444'

  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'center', gap: 16, marginBottom: 16 }}>
        {[[hrs,'Hours'],[mins,'Minutes'],[secs,'Seconds']].map(([v,l]) => (
          <div key={l} style={{ textAlign: 'center', background: '#06060F', border: '1px solid rgba(255,255,255,0.09)', borderRadius: 12, padding: '16px 20px', minWidth: 80 }}>
      ...
      <div style={{ background: '#06060F', borderRadius: 8, height: 8, overflow: 'hidden' }}>
    ...
    <div style={S.page}>
      <div style={S.wrap}>
        ...
        <div style={S.card}>
          ...
          <div style={S.row}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {DATA_TYPES.map(t => (
                <button key={t} onClick={() => toggleType(t)} style={{ background: form.dataTypes.includes(t) ? 'rgba(139,92,246,0.15)' : 'rgba(15,23,42,0.03)', border: `1px solid ${form.dataTypes.includes(t) ? '#9B7BFA' : 'rgba(255,255,255,0.09)'}`, color: form.dataTypes.includes(t) ? '#9B7BFA' : '#94A3B8', borderRadius: 20, padding: '6px 14px', fontSize: 13, cursor: 'pointer' }}>
                  {t}
                </button>
              ))}
            </div>
        ...
        <div style={S.card}>
          ...
          <div style={{ marginTop: 20, paddingTop: 20, borderTop: '1px solid rgba(255,255,255,0.09)' }}>
