  ...
  return (
    <div style={{background:S.bg,minHeight:'100vh',paddingBottom:80,fontFamily:'var(--f-head,sans-serif)'}}>
      ...
      <div style={{background:S.surface,borderBottom:`1px solid ${S.border}`,padding:'28px 48px',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:16}}>
        ...
        <button onClick={load} style={{padding:'10px 20px',background:'rgba(0,212,170,0.1)',border:`1px solid ${S.teal}`,borderRadius:10,color:S.teal,fontWeight:700,cursor:'pointer',fontSize:13}}>
          ↻ Refresh
        </button>
      </div>
          ...
          <div style={{background:S.surface,border:`1px solid ${S.border}`,borderRadius:14,padding:'20px'}}>
            ...
            {(v.topPages || []).map((p,i) => (
              <div key={i} style={{display:'flex',alignItems:'center',gap:10,marginBottom:10}}>
                ...
                <div style={{flex:1,background:'#060B14',borderRadius:6,overflow:'hidden',height:6}}>
        ...
        <div style={{display:'flex',gap:10,marginBottom:16,flexWrap:'wrap',alignItems:'center'}}>
          ...
          {['device','country'].map(f => (
            <input key={f} placeholder={f.charAt(0).toUpperCase()+f.slice(1)} value={filter[f]}
              ...
              style={{background:'#060B14',border:`1px solid ${S.border}`,borderRadius:8,padding:'7px 12px',color:S.text,fontSize:12,outline:'none',width:130}} />
          ))}
          <input placeholder="Page path" value={filter.page}
            onChange={e => { setFilter(p=>({...p,page:e.target.value})); setPage(1) }}
            style={{background:'#060B14',border:`1px solid ${S.border}`,borderRadius:8,padding:'7px 12px',color:S.text,fontSize:12,outline:'none',width:160}} />
