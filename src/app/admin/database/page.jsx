...
export default function AdminDatabase() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#060B14', color: '#E8F0FE', fontFamily: 'var(--font-space-grotesk,"Space Grotesk"),sans-serif', padding: '32px 24px' }}>

      {/* Header */}
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 32, flexWrap: 'wrap', gap: 12 }}>
          <div>
            <a href="/admin" style={{ color: 'rgba(232,240,254,0.4)', fontSize: 13, textDecoration: 'none', display: 'block', marginBottom: 6 }}>← Back to Admin</a>
            ...
            <p style={{ color: 'rgba(232,240,254,0.4)', fontSize: 13, marginTop: 4 }}>MongoDB Atlas · algograss database</p>
          </div>
          <div style={{ display: 'flex', gap: 10 }}>
            <button onClick={loadHealth} style={{ padding: '10px 18px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, color: '#E8F0FE', fontSize: 13, cursor: 'pointer' }}>
              ↻ Refresh
            </button>
            <button onClick={runInit} disabled={initing} style={{ padding: '10px 20px', background: initing ? 'rgba(0,212,170,0.3)' : A, border: 'none', borderRadius: 8, color: '#060B14', fontSize: 13, fontWeight: 700, cursor: initing ? 'wait' : 'pointer' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 16, marginBottom: 32 }}>
          {[
            ...
            <div key={s.label} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: '18px 20px' }}>
              <div style={{ fontSize: 11, color: 'rgba(232,240,254,0.4)', letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: 6 }}>{s.label}</div>
        ...
        {/* Init result */}
        {initResult && (
          <div style={{ background: 'rgba(0,212,170,0.08)', border: '1px solid rgba(0,212,170,0.2)', borderRadius: 12, padding: '16px 20px', marginBottom: 24 }}>
            <div style={{ color: A, fontWeight: 700, marginBottom: 8 }}>✅ Initialization complete — {initResult.collections} collections processed</div>
            <div style={{ fontSize: 12, color: 'rgba(232,240,254,0.4)' }}>{initResult.initializedAt}</div>
            {initResult.results && (
              <div style={{ marginTop: 12, display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {initResult.results.map(r => (
                  <span key={r.name} style={{ fontSize: 11, background: 'rgba(0,212,170,0.1)', border: '1px solid rgba(0,212,170,0.2)', borderRadius: 6, padding: '3px 8px', color: A }}>
        ...
        <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, overflow: 'hidden' }}>
          <div style={{ padding: '20px 24px', borderBottom: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <h2 style={{ fontSize: 15, fontWeight: 700, color: '#E8F0FE' }}>All Collections</h2>
            <span style={{ fontSize: 12, color: 'rgba(232,240,254,0.3)' }}>{health?.checkedAt ? `Checked ${new Date(health.checkedAt).toLocaleTimeString('en-GB')}` : ''}</span>
          </div>

          {loading ? (
            <div style={{ padding: 48, textAlign: 'center', color: 'rgba(232,240,254,0.3)' }}>Loading database health…</div>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                  {['Collection', 'Description', 'Documents', 'Indexes', 'Status'].map(h => (
                    <th key={h} style={{ padding: '12px 24px', textAlign: 'left', fontSize: 11, fontWeight: 600, color: 'rgba(232,240,254,0.35)', letterSpacing: '.08em', textTransform: 'uppercase' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {(health?.collections || []).map((col, i) => (
                  <tr key={col.name} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.015)' }}>
                    <td style={{ padding: '14px 24px', fontWeight: 600, fontSize: 13, color: A, fontFamily: 'monospace' }}>{col.name}</td>
                    <td style={{ padding: '14px 24px', fontSize: 12, color: 'rgba(232,240,254,0.4)', maxWidth: 280 }}>{col.description}</td>
                    <td style={{ padding: '14px 24px', fontSize: 14, fontWeight: 700, color: col.documents > 0 ? '#E8F0FE' : 'rgba(232,240,254,0.25)' }}>
                      {col.documents.toLocaleString()}
                    </td>
                    <td style={{ padding: '14px 24px', fontSize: 13 }}>
                      <span style={{ background: col.indexes > 1 ? 'rgba(129,140,248,0.15)' : 'rgba(255,255,255,0.05)', color: col.indexes > 1 ? P : 'rgba(232,240,254,0.3)', borderRadius: 6, padding: '2px 8px', fontSize: 12 }}>
        ...
        {/* Schema reference */}
        <div style={{ marginTop: 32, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, padding: '24px' }}>
          <h2 style={{ fontSize: 15, fontWeight: 700, marginBottom: 20, color: '#E8F0FE' }}>Schema Reference</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(300px,1fr))', gap: 16 }}>
            {[
              ...
              <div key={s.name} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10, padding: '14px 16px' }}>
                <div style={{ fontFamily: 'monospace', color: A, fontWeight: 700, fontSize: 13, marginBottom: 8 }}>{s.name}</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                  {s.fields.map(f => (
                    <span key={f} style={{ fontSize: 11, color: 'rgba(232,240,254,0.45)' }}>· {f}</span>
                  ))}
                </div>
              </div>
            ))}
        ...
        <p style={{ textAlign: 'center', marginTop: 32, fontSize: 12, color: 'rgba(232,240,254,0.2)' }}>
