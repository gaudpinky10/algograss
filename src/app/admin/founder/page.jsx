      ...
      <section style={{background:'var(--bg2)',padding:'28px 0 0',borderBottom:'1px solid var(--border)'}}>
        <div className="wrap">
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20,flexWrap:'wrap',gap:12}}>
            <div>
              <span style={{fontSize:11,fontWeight:700,color:'var(--accent)',textTransform:'uppercase',letterSpacing:'.08em'}}>Founder Dashboard</span>
          ...
          <div style={{display:'flex',gap:4,flexWrap:'wrap'}}>
            {['overview','users','tools','activity'].map(t=>(
              <button key={t} onClick={()=>setTab(t)} style={{fontSize:12,fontWeight:600,padding:'7px 16px',borderRadius:'8px 8px 0 0',border:'none',background:tab===t?'var(--bg)':'transparent',color:tab===t?'var(--accent)':'var(--ink2)',cursor:'pointer',borderBottom:tab===t?'2px solid var(--accent)':'2px solid transparent'}}>
                      ...
                      {Object.entries(signupsByDay||{}).slice(-20).map(([d,c])=>{
                        ...
                        return(
                          <div key={d} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:3}}>
                            <div title={`${d}: ${c} signups`} style={{width:'100%',background:'var(--accent)',borderRadius:'2px 2px 0 0',height:`${Math.round(c/max*64)+4}px`,minHeight:4}}/>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:8,marginBottom:28}}>
                {(topUsers||[]).map((u,i)=>(
                  <div key={u.email} style={{background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:10,padding:'12px 18px',display:'flex',alignItems:'center',gap:14}}>
                    <span style={{fontFamily:'Syne,sans-serif',fontSize:18,fontWeight:800,color:'var(--accent)',minWidth:32}}>#{i+1}</span>
                    <span style={{flex:1,fontSize:13,color:'var(--ink)'}}>{u.email}</span>
                    <span style={{fontSize:12,color:'var(--ink2)'}}>{u.count} actions</span>
                    <div style={{width:80,height:6,background:'rgba(255,255,255,0.07)',borderRadius:3,overflow:'hidden'}}>
                      <div style={{height:'100%',width:`${Math.round(u.count/(topUsers[0]?.count||1)*100)}%`,background:'var(--accent)',borderRadius:3}}/>
                    </div>
                  </div>
                ))}
              ...
              <div style={{background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:14,padding:'20px 22px'}}>
                ...
                {Object.entries(toolUsage||{}).sort((a,b)=>b[1]-a[1]).map(([tool,count])=>(
                  <div key={tool} style={{display:'flex',alignItems:'center',gap:10,marginBottom:12}}>
                    ...
                    <div style={{flex:1,height:8,background:'rgba(255,255,255,0.07)',borderRadius:4,overflow:'hidden'}}>
                      <div style={{height:'100%',width:`${totalToolUses?Math.round(count/totalToolUses*100):0}%`,background:TOOL_COLOR[tool]||'var(--accent)',borderRadius:4}}/>
                    </div>
                    <span style={{fontSize:12,fontWeight:700,color:TOOL_COLOR[tool]||'var(--accent)',minWidth:40,textAlign:'right'}}>{count}</span>
                  </div>
                ))}
                {!Object.keys(toolUsage||{}).length&&<p style={{color:'var(--ink2)',fontSize:13}}>No tool usage yet</p>}
              </div>
              <div style={{display:'flex',flexDirection:'column',gap:14}}>
                {Object.entries(toolUsage||{}).sort((a,b)=>b[1]-a[1]).slice(0,4).map(([tool,count])=>(
                  <div key={tool} style={{background:'var(--bg2)',border:`1px solid ${TOOL_COLOR[tool]||'var(--border)'}40`,borderRadius:12,padding:'16px 20px',borderLeft:`3px solid ${TOOL_COLOR[tool]||'var(--accent)'}`}}>
                    <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}>
                      <span style={{fontSize:13,fontWeight:600,color:'var(--ink)'}}>{TOOL_ICON[tool]||'⚡'} {tool}</span>
                      <span style={{fontFamily:'Syne,sans-serif',fontSize:20,fontWeight:800,color:TOOL_COLOR[tool]||'var(--accent)'}}>{count}</span>
                    </div>
                    <div style={{fontSize:11,color:'var(--ink2)'}}>{totalToolUses?Math.round(count/totalToolUses*100):0}% of all tool uses</div>
                  </div>
                ))}
              </div>
            </div>
              ...
              {Object.keys(actsByDay||{}).length===0?<p style={{color:'var(--ink2)',fontSize:13}}>No activity yet</p>:(
                <div style={{display:'flex',alignItems:'flex-end',gap:3,height:100,marginBottom:8}}>
                  {Object.entries(actsByDay||{}).slice(-30).map(([d,c])=>{
                    ...
                    return(
                      <div key={d} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:3}}>
                        <div title={`${d}: ${c} actions`} style={{width:'100%',background:'var(--accent)',borderRadius:'2px 2px 0 0',height:`${Math.round(c/max*80)+4}px`,minHeight:4,opacity:.8}}/>
