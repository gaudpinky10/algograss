...
export default function SignupPage() {
  ...
  async function submit(e) {
    ...
    try {
      ...
      // If Stripe is configured, go to checkout
      if (form.plan !== 'free') {
        const checkout = await fetch('/api/checkout', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ plan: form.plan, email: form.email }),
        })
      ...
      <div style={{ width: '100%', maxWidth: 480 }}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          <h1 style={{ fontFamily: 'Syne,sans-serif', fontSize: 24, fontWeight: 700, color: 'var(--ink)', marginBottom: 5 }}>
            {step === 1 ? 'Create your free account' : 'Choose your plan'}
          </h1>
          <p style={{ fontSize: 13, color: 'var(--ink2)' }}>{step === 1 ? 'No credit card required to start.' : 'Cancel any time. Upgrade or downgrade easily.'}</p>
        </div>
        <div style={{ display: 'flex', gap: 6, marginBottom: 24 }}>
          {[1, 2].map(n => <div key={n} style={{ flex: 1, height: 3, borderRadius: 2, background: n <= step ? 'var(--green)' : 'var(--green-m)', transition: 'background .3s' }} />)}
        </div>
        <div className="card" style={{ padding: '30px 26px' }}>
          <form onSubmit={submit}>
            ...
            <button type="submit" className="btn btn-primary btn-full" disabled={loading}>
              {loading ? 'Setting up your account...' : step === 1 ? 'Continue →' : 'Start subscription →'}
            </button>
          </form>
        </div>
